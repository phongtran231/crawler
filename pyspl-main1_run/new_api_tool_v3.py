    ...
    try:
        for i in range(len(inlxxx)):
            if str(listi[0]) == str(inlxxx[i][0]):
                if  str(listi[1]) == str(inlxxx[i][1]) and str(listi[2]) == str(inlxxx[i][2]) and str(listi[3]) == str(inlxxx[i][3]) and str(listi[4]) == str(inlxxx[i][4]) and str(listi[5]) == str(inlxxx[i][5]) and str(listi[6]) == str(inlxxx[i][6]) and str(listi[7]) == str(inlxxx[i][7]):
                    return i
    except:
        return -1
    return -1
...
#        print(s)
        ...
        #f.close()   
        i = -1
        i = find_index_in_list([mana_cap,summoner_id,id1,id2,id3,id4,id5,id6,ruleset.split('|')[0],0,0,0])
        ...
        if i < 0:
            if is_win == 1:
                final_list.append([mana_cap,summoner_id,id1,id2,id3,id4,id5,id6,ruleset.split('|')[0],1,0,1])
            else:
                final_list.append([mana_cap,summoner_id,id1,id2,id3,id4,id5,id6,ruleset.split('|')[0],0,1,1])
        else:
            if is_win == 1:
...
#        print(f'{mana_cap},{summoner_id},{id1},{id2},{id3},{id4},{id5},{id6},{ruleset}\n')
        ...
        if summoner_id == '145' and id1== '412' and id2=='426' and id3 =='414':
            ...
            ftg.write([mana_cap,summoner_id,id1,id2,id3,id4,id5,id6,ruleset.split('|')[0],0,1,1])
            ftg.write('\n')
            ftg.write(id)
            ftg.write('\n-----------------')
            ftg.write(data)
        ...
        if tmp_str == item[8]  and i != lenfna:
            ...
            for itemx in myli:
                if manax == '':
                    ...
                    if manax == itemx[0] and i != lenfna:
                        myli2.append(itemx)
                    else:
                        filename = 'result/'+itemx[8] + manax + 'mana' + '.JSON'
                        ...
                        if 1==1:
                            ...
                            for xxuu in myli2:
                                ...
                                if flc < 0:
                                    ...
                                    print(out_list)
                                    print('-------------------')
                                    print([xxuu[1],xxuu[2],xxuu[3],xxuu[4],xxuu[5],xxuu[6],xxuu[7],xxuu[8],xxuu[0]])
                                else:
                                    
                                    axxxx = out_list[flc][9]
                                    out_list[flc][9] = out_list[flc][9] + xxuu[11]
                                    out_list[flc][10] = (xxuu[9] + axxxx*out_list[flc][10]/100)/out_list[flc][9]
